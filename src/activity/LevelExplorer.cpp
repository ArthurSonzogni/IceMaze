...
LevelExplorer::LevelExplorer(smk::Window& window) : Activity(window) {}

void LevelExplorer::Draw() {
  smk::View view;
  view.SetCenter(window().width() / 2, yview);
  ...
  if (window().input().IsKeyPressed(GLFW_KEY_UP))
    choice--;

  if (window().input().IsKeyPressed(GLFW_KEY_DOWN))
    choice++;
  ...
  float yview_target = choice * 40 + window().height() * 0.5;
  yview += (yview_target - yview) * 0.05;

  // dessin de l'arrièreplan
  for (int y = yview / 32 - window().height() / 32;
       y <= yview / 32 + window().height() / 32; y++) {
    for (int x = 0; x <= window().width() / 32; x++) {
...
void LevelExplorer::OnEnter() {
  yview = choice * 40 + window().height() * 0.5;
}
